...
obabel -ixyz $FNAME -oINCHIKEY --errorlevel 0 2>&1  | grep -e "-" | tail -1 > inchikey1.txt

# calculate optimized structure and inchikey2; no output (only bash)
xtb $FNAME --opt  &> /dev/null

# calculate second inchikey2
obabel -ixyz xtbopt.xyz -oINCHIKEY  ---errorlevel 0 2>&1  | grep -e "-" | tail -1 > inchikey2.txt
